...
## Workplaces
This section reports data from IDBR on local units, which are defined by ONS as "an individual site (for example a factory or shop) within an enterprise". We refer to these local units as "workplaces".
...
reactable(
  ...
  columns = list(
    ...
    `% of total employees` = colDef(footer = function(values) {
      ...
      format = colFormat(separators = TRUE, digits = 0),
      footerStyle = list(fontWeight = "bold")),
    `Workplace size band` = colDef(footer = function(values) {
      format(sum(values), big.mark = ",")
      }, 
      format = colFormat(separators = TRUE, digits = 0),
      footerStyle = list(fontWeight = "bold"))
...
production_workplaces_fig <- plot_ly(production_workplaces_fig, 
  ...
  text = ~paste("Year:", format(Year, "%Y"), 
                "<br>% of all employees:", percentage, 
  ...
  layout(
    ...
    yaxis = list(
      title = "Percentage of all employees",
